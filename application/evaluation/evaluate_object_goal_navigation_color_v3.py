...
import hydra
import logging
from vlmaps.task.habitat_object_nav_task_across import HabitatObjectNavigationTaskAcross
    ...
    for scene_i, scene_id in enumerate(scene_ids):
        ...
        for task_id in range(len(object_nav_task.task_dict)):
            ...
            for cat_i, (cat, color, obj_info) in enumerate(zip(object_categories, colors_rgb, object_nav_task.objects_info)):
                ...
                # 执行移动到物体的动作
                actions_list = robot.move_to_color_3d_object(cat, color, color_values_gt, vis = config.nav.vis)
